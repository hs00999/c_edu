...
{
	int up[4], down[4], sum[4];
	int sta,i,temp, MAX=0;
	for(sta=0;sta<4;sta++){
	scanf("%d %d", &down[sta], &up[sta]);
	}
	...
	for(sta=0;sta<4;sta++){
		if(sta==0) {
 		...
 		sum[sta] = sum[sta-1] + up[sta] - down[sta];
		if(sum[sta] > 10000) printf("정원을 초과하였습니다.");
	}

	
	for(sta=0;sta<4;sta++){
		if(MAX<sum[sta]) MAX = sum[sta];
	}
