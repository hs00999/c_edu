...
int Sort_Select(SCORE * d, int order, int (*comp)(SCORE * x, SCORE * y))
{
	...
	for(i = max -1 ; i > 0 ; i++)
	{
		for(j=0;j<i;j++)
		{
			if(comp(&d[j], &d[j+1]) * order >0) //if(comp(&d[j], &d[idx]) * order >0)
			{
				idx = d[i];
			}
			temp = d[i];
...
int Copy_Data(int n, SCORE * p)
{
	//if((unsigned int)n >= MAX_ST) return -1;   ---> 요소범위 벗어남
	if(exam[n] == 0) return -1;//if(exam[n].id == 0) return -1;
	exam[n].id = p->id;
	exam[n].name = p->name;
	exam[n].jumsu = p->jumsu;
	//*p = exam[n];
	return 0;
...
/***********************************************************/
 ...
 int Print_All_Buffer(SCORE * d, int max)
 {
	...
	for(i=0 ; i<max ;i++) printf("ID: %d, Name: %s, Score: %d\n", d[i].id, d[i].name, d[i].jumsu);
	if(cnt < max) {
		for(i=0 ; i<cnt ;i++) printf("ID: %d, Name: %s, Score: %d\n", d[i].id, d[i].name, d[i].jumsu);
	}
	return max;
}
